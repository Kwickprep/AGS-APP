...
/// Auto Reply list screen with full features: filter, sort, pagination, and details
...
class _AutoReplyScreenState extends State<AutoReplyScreen> {
  ...
  void _showDetails(WhatsAppAutoReplyModel record) {
    DetailsBottomSheet.show(
      ...
      fields: [
        ...
        DetailField(label: 'Trigger Message', value: record.userMessage),
        DetailField(label: 'Type', value: record.autoReplyType),
        if (record.messageContent != null && record.messageContent!.isNotEmpty)
          DetailField(label: 'Message Content', value: record.messageContent!),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          PermissionWidget(
            ...
            child: IconButton(
              onPressed: () async {
                final result = await Navigator.pushNamed(context, '/whatsapp-auto-replies/create');
      ...
      body: BlocProvider<AutoReplyBloc>(
        create: (_) => _bloc,
        child: SafeArea(
          child: Column(
            children: [
              ...
              // Card list
              Expanded(
                child: BlocBuilder<AutoReplyBloc, AutoReplyState>(
                  builder: (context, state) {
                    if (state is AutoReplyLoading) {
                      return const Center(child: CircularProgressIndicator());
                    } else if (state is AutoReplyError) {
                      return Center(
                        child: Column(
                      ...
                      if (state.records.isEmpty) {
                        return Center(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              const Icon(Icons.reply_all_outlined, size: 64, color: AppColors.grey),
                              const SizedBox(height: 16),
                              Text(
  ...
  Widget _buildRecordCard(WhatsAppAutoReplyModel record, int serialNumber) {
    return RecordCard(
  ...
  void _confirmDelete(WhatsAppAutoReplyModel record) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              _bloc.add(DeleteAutoReply(record.id));
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Auto reply deleted successfully')),
              );
            },
